...
func (k BaseKeeperWrapper) FeeToReceivers(ctx sdk.Context, inputs []banktypes.Input, outputs []banktypes.Output, receiverTypes []types.FeeReceiverType) error {
	...
	for _, output := range outputs {
		receiverAddr, err := sdk.AccAddressFromBech32(output.Address)
		if err != nil {
			return sdkerrors.Wrapf(sdkerrors.ErrInvalidAddress, "invalid fee receiver address %s", output.Address)
		}

		if k.BlockedAddr(receiverAddr) {
			return sdkerrors.Wrapf(sdkerrors.ErrUnauthorized, "%s is not allowed to receive funds", output.Address)
		}
	}
