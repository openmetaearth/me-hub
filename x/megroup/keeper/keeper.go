...
func (k Keeper) KycStatusChanged(goCtx context.Context, msgType string, data interface{}) error {
	...
	if val, ok := data.(sdk.Event); !ok {
		return errors.New("data's type is not sdk.Event.but msgType is update")
	} else {
		attrPreRegion, found := val.GetAttribute(kyctypes.AttributeKeyRegionId)
		if !found {
			return errors.New("can not found AttributeKeyRegionId.but EventType is update")
		}
		attrNewRegion, found := val.GetAttribute(kyctypes.AttributeKeyRegionIdChanged)
		if !found {
			return errors.New("can not found AttributeKeyRegionIdChanged.but EventType is update")
		}
		...
		if !found {
			return errors.New("can not found AttributeKeyAddress.but EventType is update")
		}
...
func (k Keeper) procKycRegionChange(sdkCtx sdk.Context, address, preRegionID, nowRegionID string) error {
	...
	if !found { // if new group has not been created,emit event and return
		return errorsmod.Wrapf(types.ErrGroupNotExist, "can not found group by groupID.groupID = %d", newGrpId)
	}
	newGrpNumberCnt, found := k.GetGroupMemberCount(sdkCtx, newGrpId)
	if !found {
		return errors.New("can not found newGroup number count while ready to join newGourp in procKycRegionChange")
	}
	...
	if !joinGroupFound { // send rewards if user has not joined group
		...
		if !found {
			return errorsmod.Wrapf(types.ErrRegionNotExist, "group's region = %s", nowRegionID)
		}
		...
		if err != nil {
			return errorsmod.Wrap(types.ErrProcData, fmt.Sprintf("transfer rewards coins error. err = %s,fromAddr = %s,toAddr = %s",
		...
		if err != nil {
			return errorsmod.Wrapf(types.ErrProcData, "transfer rewards coins error. err = %s,fromAddr = %s,toAddr = %s",
...
func (k Keeper) CreateGroupByRegion(sdkCtx sdk.Context, regionInfo stakingtypes.Region) (uint64, error) {
	...
	if found {
		return 0, errorsmod.Wrap(types.ErrGroupExceededInRegion, fmt.Sprintf("group of region has been created.groupId = %d", preGroupID))
	}
