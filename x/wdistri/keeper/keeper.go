...
func (k Keeper) AllocateBlockReward(ctx sdk.Context) error {
	...
	for _, region := range regions {
		...
		regionTreasuryAddress, err := sdk.AccAddressFromBech32(region.GetRegionTreasureAddr())
		if err == nil {
			if err = k.bankKeeper.SendCoinsFromModuleToAccount(ctx, k.feeCollectorName, regionTreasuryAddress, regionCoins); err != nil {
				return err
			}
			ctx.EventManager().EmitEvents(sdk.Events{
				sdk.NewEvent(
					types.EventTypeRegionTreasuryReward,
					sdk.NewAttribute(types.AttributeKeyRegionTreasuryAddress, regionTreasuryAddress.String()),
