...
func NewIbcTransferFromRegionTreasureCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "ibc-transfer-from-region-treasure [src-port] [src-channel] [region] [amount] [receiver]",
...
corresponding to the counterparty channel. Any timeout set to 0 is disabled.`),
		Example: fmt.Sprintf("%s tx ibc-transfer-from-region-treasure transfer [src-port] [src-channel] [region] [amount] [receiver]", version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgIbcTransferFromRegionTreasure(
				srcPort,
				srcChannel,
				region,
				coin,
