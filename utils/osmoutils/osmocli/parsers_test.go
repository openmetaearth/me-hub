...
func TestParseFieldFromArg(t *testing.T) {
	tests := map[string]struct {
		...
		"Struct (sdk.Coin) change": {
			testingStruct:  testingStruct{Struct: sdk.NewCoin("bar", math.NewInt(10))}, // only supports sdkmath.Int, sdk.Coin or time.Time, other structs are not recognized
			arg:            "100bar",
			fieldIndex:     7,
			expectedStruct: testingStruct{Struct: sdk.NewCoin("bar", math.NewInt(10))},
		},
		"Unrecognizable struct": {
			testingStruct: testingStruct{Struct: testingStruct{}}, // only supports sdkmath.Int, sdk.Coin or time.Time, other structs are not recognized
			arg:           "whatever",
			fieldIndex:    7,
			expectingErr:  true,
		},
